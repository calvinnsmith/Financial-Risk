...
maximas06 <- blockMaxima(ret2006,T)
scale06 <- gpfit06$mle[1]
shape06 <- gpfit06$mle[2]
rate06 <- gpfit06$rate

maximas20 <- blockMaxima(ret2020,T)
scale20 <- gpfit20$mle[1]
shape20 <- gpfit20$mle[2]

K <- (rate06*T)^shape06
l <- scale06/shape06

prob_06 <- as.vector(exp(-(1 + shape06*((maximas06 - mu06 - (K -1)*l)/(scale06*K)))^(-1/shape06)))
prob_06 <- sort(prob_06)

maximas06 <-sort(as.vector(maximas06),decreasing = TRUE)



mu_new <- mu06 + ((rate06*T)^(shape06) - 1)*(shape06/scale06)

scale_new <- scale06*(rate06*T)^(shape06)

qgev(0.01,xi = shape06,mu = mu_new,beta = scale_new, lower.tail = FALSE)
